const {CarWash,validateCarWash,validateUpdateCarWash}=require('../models/carWashModel');
...
exports.creatCarWash = asyncError(async(req,res)=>{  
    ...
    if(error)return res.status(404).json({
        ...
        if(validate)return res.status(400).json({
            status:'failed',
            message:'You are already registered ..! if you want to update your information you can go to your profile !! '
        })
    ...
    exports.updateCarWash=asyncError(async(req,res)=>{
         ...
         const carWash = await CarWash.findOneAndUpdate({userId:req.user._id},req.body,{new:true})
