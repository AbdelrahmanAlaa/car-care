const { Worker,validateRestPassword,validateUpdate} = require('../models/workerModel');
...
exports.updatePassword=asyncError(async(req,res)=>{
    ...
    const checkPassword = await bcrypt.compare(req.body.currentPassword,worker.password);
    if(!worker||!checkPassword)return res.status(404).json({
        status:'failed',
        message:'incorrect password ... '
    });

    const {error} = validateRestPassword({password:req.body.newPassword,confirmPassword:req.body.confirmPassword});
...
exports.updateMe=asyncError(async(req,res)=>{
    ...
    if(error)return res.status(400).json({
        ...
        const worker = await Worker.findByIdAndUpdate(req.worker._id, req.body , {new:true});
